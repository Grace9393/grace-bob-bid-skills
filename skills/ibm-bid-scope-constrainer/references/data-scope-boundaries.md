...
- `1` What source and scale boundaries are required to keep the data scope finite?
  - `1.1` Which source systems, interfaces, file formats, and ingestion methods are explicitly included, and which additional sources are out of scope?
  - `1.2` What limits must be stated for record counts, storage volumes, event throughput, retention windows, and growth assumptions?
  - `1.3` Which environments, historical periods, and cutover windows are assumed to be available for migration, backfill, and validation?
  - `1.4` What source-system dependencies, access prerequisites, and client-owned remediation tasks need to be explicit before delivery starts?
- `2` What quality and transformation boundaries are required to prevent open-ended cleansing work?
  - `2.1` Which data-quality checks are included, such as format validation, null checks, and deduplication, and which business-rule remediation remains client responsibility?
  - `2.2` What limits should be set on transformation complexity, custom code, enrichment logic, and derived metrics?
  - `2.3` Which assumptions about master data, reference data, and business definitions must be fixed to avoid redesign during build?
  - `2.4` What restrictions on schema change, source volatility, and late-arriving data should trigger formal change control?
- `3` What architecture and operating-model boundaries are required to control build and run effort?
  - `3.1` Which target architecture choices for warehouse, lake, lakehouse, ETL tooling, modeling pattern, and analytics platform need to be fixed?
  - `3.2` What limits should apply to entities, attributes, pipelines, layers, dashboards, reports, and metadata coverage?
  - `3.3` Which backup, recovery, synchronization, and performance expectations need explicit cutoffs for validation depth, rollback, and tuning effort?
  - `3.4` What real-time, streaming, MDM, and feature-store expectations are included versus explicitly excluded?
- `4` What governance and commercial boundaries are required to avoid hidden delivery obligations?
  - `4.1` Which security, classification, masking, access-control, and audit requirements are in scope, and which advanced controls are excluded?
  - `4.2` What ownership split should be stated for stewardship, business metadata, golden-record decisions, and data-quality remediation?
  - `4.3` Which post-go-live support, monitoring, documentation, and operational reporting commitments are included versus excluded?
  - `4.4` Which trigger conditions should force re-estimation, such as new domains, higher retention, stronger DR requirements, real-time expansion, or unresolved source issues?
...
### 16. Business Semantics and Definition Boundaries

**Bounding Statements:**
- "Business glossary: [number] priority terms defined; enterprise-wide glossary excluded"
- "KPI definitions: [specific KPI list] only; new measures require change control"
- "Metric ownership: client business owners approve all calculations"
- "Semantic-layer design: limited to [specific subject areas]"
- "Historic KPI restatement: excluded unless explicitly scoped"

**Risk Mitigation:**
- Prevents endless debate on definitions during build
- Clarifies scope of semantic alignment
- Limits metric proliferation
- Protects against retroactive rework
- Establishes business ownership of meaning

### 17. Environment and Release Boundary Conditions

**Bounding Statements:**
- "Environments: [dev / test / prod] only; additional environments excluded"
- "Release cadence: [weekly / fortnightly] planned releases"
- "Deployment approvals: client-owned change approval process"
- "Release rollback support: one rollback path per release"
- "Parallel releases across multiple business units: excluded"

**Risk Mitigation:**
- Sets clear environment estate
- Clarifies release-management expectations
- Limits deployment coordination overhead
- Protects against complex multi-track delivery
- Defines rollback scope

### 18. Data Access and Consumption Boundaries

**Bounding Statements:**
- "Consumer groups: [number] named user groups only"
- "Access methods: SQL, dashboards, and scheduled extracts only"
- "Self-service modeling: excluded; curated consumption layer only"
- "Extract generation: maximum [number] recurring exports"
- "External data sharing: excluded unless explicitly scoped"

**Risk Mitigation:**
- Prevents uncontrolled consumer sprawl
- Clarifies supported access patterns
- Limits self-service governance burden
- Protects against ad hoc extract growth
- Defines sharing boundaries clearly

### 19. Lifecycle and Archival Boundaries

**Bounding Statements:**
- "Active data retention: [period]; archival beyond this is client responsibility"
- "Purge rules: [specific datasets] only"
- "Legal hold handling: client-defined and client-approved"
- "Cold storage retrieval SLA: excluded from base scope"
- "Historical backfill beyond agreed window: change request required"

**Risk Mitigation:**
- Defines lifecycle-management scope
- Clarifies archival ownership
- Limits long-tail storage obligations
- Protects against unplanned retrieval work
- Prevents silent expansion of historical scope

### 20. Operational Support and Stewardship Boundaries

**Bounding Statements:**
- "Operational support: pipeline monitoring and failure triage only"
- "Business data stewardship: client responsibility"
- "Data issue resolution: IBM identifies defects; source correction client responsibility"
- "Support window: [business hours / 24x5] only"
- "Managed data-ops service: excluded unless separately scoped"

**Risk Mitigation:**
- Prevents implied end-to-end ownership of data operations
- Clarifies stewardship accountability
- Limits support obligations
- Protects against permanent managed-service creep
- Separates technical fixes from business remediation
