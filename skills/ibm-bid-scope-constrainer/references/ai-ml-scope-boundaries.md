...
- `1` What problem-definition boundaries are required to keep the AI/ML scope commercially and technically controllable?
  - `1.1` Which business decisions, user journeys, and operational use cases are explicitly in scope, and which adjacent use cases are excluded?
  - `1.2` What model tasks, output formats, and automation behaviours must be constrained so the solution does not drift across classification, forecasting, recommendation, and generative use cases?
  - `1.3` What success measures, acceptance thresholds, and fallback positions should be stated to avoid implicit commitment to unrealistic performance outcomes?
  - `1.4` Which edge cases, exception classes, and human-review checkpoints must be defined so the AI/ML solution is not assumed to work universally?
- `2` What data and feature boundaries are required before delivery can be estimated safely?
  - `2.1` Which data sources, labeled examples, historical periods, and refresh patterns are assumed to exist at project start?
  - `2.2` What minimum standards for completeness, quality, class balance, and feature availability need to be stated as client prerequisites?
  - `2.3` Which data preparation activities are included, such as normalization and limited feature engineering, and which activities like large-scale labeling or enrichment are excluded?
  - `2.4` What restrictions on sensitive data, PII, regulated content, and external datasets must be made explicit before solution design begins?
- `3` What delivery and runtime boundaries are required to contain implementation effort?
  - `3.1` Which model frameworks, cloud platforms, compute environments, and serving patterns are locked, and which alternative stacks are excluded?
  - `3.2` What limits should be set on experimentation cycles, hyperparameter tuning, prompt iteration, fine-tuning, and custom model development?
  - `3.3` Which non-functional commitments for latency, throughput, availability, retraining cadence, and monitoring need explicit caps or exclusions?
  - `3.4` What MLOps obligations for pipelines, versioning, approvals, rollback, and incident handling are included versus excluded?
- `4` What governance, risk, and commercial boundaries are required to avoid hidden obligations?
  - `4.1` Which explainability, bias, fairness, and model-governance requirements are mandatory, and which enhanced assurance activities are excluded?
  - `4.2` What security, privacy, regulatory, and intellectual-property obligations sit with the client versus the supplier?
  - `4.3` Which support and operating-model assumptions for human oversight, model review, and business ownership need to be fixed before contract commitment?
  - `4.4` Which trigger conditions should force re-scoping, such as insufficient data, new use cases, stronger compliance requirements, or demands for autonomous decision-making?
...
### 16. Human Oversight and Decisioning Boundaries

**Bounding Statements:**
- "Human-in-the-loop required for [specific decision types]; full autonomous decisioning excluded"
- "Manual review thresholds: predictions below [confidence]% routed for human review"
- "Business override capability: included for [specific roles] only"
- "Decision accountability: final business decision remains client responsibility"
- "Appeals/dispute workflow: excluded unless explicitly scoped"

**Risk Mitigation:**
- Prevents implied autonomous-operation obligations
- Clarifies accountability for consequential decisions
- Limits exception-handling workflow scope
- Protects against unbounded review-process design
- Sets clear responsibility split

### 17. Evaluation Dataset and Acceptance-Test Boundaries

**Bounding Statements:**
- "Acceptance testing dataset: client-provided representative sample of [number] records"
- "Ground truth labeling: client responsibility; relabeling excluded"
- "Evaluation period: one formal acceptance cycle plus [number] re-runs maximum"
- "Acceptance criteria measured against agreed test set only; live-production variance excluded"
- "Adversarial or red-team testing: excluded unless explicitly scoped"

**Risk Mitigation:**
- Prevents moving-target acceptance criteria
- Clarifies ownership of ground truth
- Limits repeated re-evaluation cycles
- Protects against production-only success definitions
- Establishes bounded validation effort

### 18. Third-Party Model and Vendor Dependency Boundaries

**Bounding Statements:**
- "Foundation model/provider: [specific provider/model family] only; provider changes require reassessment"
- "Third-party APIs: maximum [number] external AI services"
- "Vendor SLAs, quotas, and price changes: client accepted dependencies"
- "Fine-tuning on third-party hosted models: excluded unless explicitly scoped"
- "Fallback between model providers: excluded; single primary provider assumed"

**Risk Mitigation:**
- Prevents provider sprawl
- Clarifies external dependency exposure
- Limits unpriced multi-vendor architecture
- Protects against hidden quota and cost risk
- Avoids implicit resiliency commitments

### 19. Prompt, Policy, and Guardrail Boundaries

**Bounding Statements:**
- "Prompt set: maximum [number] production prompt templates"
- "Guardrails: basic content filtering and system instructions only"
- "Policy rules: [number] approved business rules/checks maximum"
- "Prompt optimization rounds: [number] iterations included"
- "Domain-specific safety policy authoring: client responsibility unless separately scoped"

**Risk Mitigation:**
- Limits open-ended prompt engineering
- Clarifies guardrail implementation scope
- Prevents uncontrolled rule accumulation
- Protects against endless tuning cycles
- Defines policy ownership clearly

### 20. Support and Service Boundary Conditions

**Bounding Statements:**
- "Support window: [business hours / 24x5] only; 24x7 excluded"
- "Incident scope: model availability and inference failures only; business-outcome disputes excluded"
- "Hypercare period: [number] weeks post-go-live"
- "Model recalibration outside planned retraining cycle: change request required"
- "Ongoing prompt/model performance optimization: excluded from base scope"

**Risk Mitigation:**
- Prevents implied managed-service coverage
- Clarifies supportable incident types
- Limits post-go-live obligations
- Protects against continuous free optimization
- Establishes support cadence and boundaries
